    ...
    .then(function (query) {
      ...
      if (resultados.length > 0) {
        ...
        resultados.map(function (item) {
          if (!lista.includes(item.genero)) return lista.push(item.genero);
        });
  ...
  try {
    ...
    usuarios
      ...
      .then(function (resultado) {
        if (!resultado.empty) resposta.sendStatus(202);
        else resposta.sendStatus(401);
      })
      .catch(function (erro) {
        resposta.status(500).json({ mensagem: erro.message });
      });
...
//Cadastrar usuário
rotas.post("/usuarios", async function (requisicao, resposta) {
  const corpo = requisicao.body;
  ...
  try {
    ...
    usuarios
      ...
      .then(function (referencia) {
        resposta.status(201).json({ id: referencia.id });
      })
      .catch(function (erro) {
        resposta.status(500).json({ mensagem: erro.message });
      });
